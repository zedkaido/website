<script>
 import PostTemplate from '../PostTemplate.svelte';
</script>

<svelte:head>
    <title>Zed Kaido | Second Rendez-Vous (Redux): A Journey Through Space & Sound</title>
</svelte:head>

<PostTemplate
    category="music-releases"
    thumbnail="/blog/second-rendez-vous-redux/thumbnail.webp"
    thumbnailAlt="Cover art for Second Rendez-Vous"
    author="Zed Kaido"
    formattedDate="23 Jan 2023"
    >
    <div id="title" slot="title">
        <h1>Second Rendez-Vous (Redux): A Journey<br/>Through Space & Sound</h1>
    </div>
    <div slot="content">
        <p>
            When it comes to classic tracks, there are few that have stood the test of time quite like <a href="https://open.spotify.com/track/31OX2Wq5revTT0UzpwPJtG?si=b6f1e1ac81a14b0e">"Second Rendez-Vous"</a> by <a href="https://jeanmicheljarre.com/">Jean Michel Jarre</a>. This iconic piece, released in 1986, was a groundbreaking composition that fused together elements of electronic, ambient and space music, creating a unique and timeless sound. The track was a commercial and critical success, and it solidified Jarre's position as one of the pioneers of electronic music.
        </p>

        <h2>A Modern Twist on a<br/> Classic Track</h2>
        <p>
            As a long-time fan of the track, I wanted to put my own spin on it, and bring it to a new audience. I wanted to create a modern, upbeat danceable twist, packed with soaring synths, punchy bass, and driving beats. My vision was to create a track that would transport listeners to another dimension, a journey through space and sound.
        </p>

        <h3>Bringing an Amazing Melody to the Big Festivals and Clubs</h3>
        <p>
            Music production has come a long way since the release of the original track, and I had access to a wide range of tools and technology that wasn't available back in the 80s. I wanted to take advantage of these advancements and create a modern interpretation of the track that would stand out from the crowd. I wanted to bring an amazing melody to the big festivals and clubs, and make it stand out from the often repetitive and uninteresting tracks that dominate the charts today.
        </p>

        <h3>The Process of Creating "Second Rendez-Vous (Redux)"</h3>
        <p>
            The process of creating "Second Rendez-Vous (Redux)" was a challenging and rewarding experience. I spent countless hours in the studio, working on every detail, and experimenting with different sounds and effects. The final track is 7:27 minutes long, divided into seventeen sections, each with its own story and emotion - I could tell you about each one of them, but I believe it comes to you to define the meaning and the story of the song:
        </p>

        <iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/3yEOjWzIpzqB3WfqfzKyjH?utm_source=generator&theme=0" width="100%" height="352" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
        <div class="spacer"/>

        <h3>A Complex and Intricate Composition</h3>
        <p>
            The soaring synths, punchy bass, driving beats and the risers and fallers create a sense of movement, tension and propulsion that's impossible to resist. Moreover, the track features a variety of electronic sounds and effects that are woven together to create a rich and dynamic soundscape.
        </p>

        <p>
            The track is designed to take the listener on a journey through different emotions and moods, each section of the track has its own story and feeling, and it seamlessly flows from one section to another. The result is a track that's not only danceable but also emotionally engaging and thought-provoking.
        </p>

        <p>
            The track features 66 layers, making it a truly intricate and immersive composition. Each layer adds something unique to the track, and it's designed to be listened to multiple times, revealing new elements and details with each listen. The track is not only a feast for the ears but also for the mind, it's a track that I believe will keep you engaged and interested for a long time.
        </p>

        <!-- <iframe src="https://youtu.be/MJ5nhiqgyLU"> -->
        <iframe width="100%" height="200px" src="https://www.youtube.com/embed/TMMxn9-0eLs" frameborder="0" allowfullscreen>
        </iframe>

        <div class="spacer"/>

        <h2>Experience the Track in Style with Limited-Edition Merchandise</h2>
        <p>
            In addition to the release of "Second Rendez-Vous (Redux)", I am super excited to unveil my limited-edition line of merchandise designed specifically for this release. From exclusive t-shirts and backpacks to tumblers and other accessories featuring mesmerizing patterns of the track cover, each item is designed to be a stylish and functional accessory for any music lover. Whether you're heading to a festival, hitting the club, or just hanging out with friends, this merchandise will help you rep your love for the track in style.
        </p>
        <p>The merchandise is designed to perfectly capture the spirit of the track, and it's a great way to show your support and appreciation for the music. You can find it in <a href="/merch">/merch</a> - Grab it before it's gone!
        </p>
        <div style="display:flex;">
            <a href="/merch">
                <img class="merchAdvert" alt="Zed Kaido's Second Rendez-Vous (Redux) Merch" src="/blog/second-rendez-vous-redux/merch-01-advert.webp"/>
            </a>
        </div>
        <div class="spacer"/>
    </div>
</PostTemplate>

<style>
 .merchAdvert {
     width: 100%;
 }
</style>
